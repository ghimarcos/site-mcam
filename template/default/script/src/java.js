$(document).ready(function () {
  ...
  function showHomepage() {
    ...
    $("#pagina-contato, #resultado-produtos, #paginacao").hide();

    // CORREÇÃO: Mostra apenas os elementos da home, especificando para NÃO mostrar a div de resultados.
    $("#destaque_full, #conteudo > .cont > .titulo").show();
    $(".listar_produtos:not(#resultado-produtos)").show(); // Este seletor agora exclui a div de resultados
  }

  function showContactPage() {
    // Esconde todas as outras seções
    $(
      "#destaque_full, .titulo, .listar_produtos, #resultado-produtos, #paginacao"
    ).hide();

    // Mostra apenas a página de contato
    $("#pagina-contato").show();
  }

  function showFilteredResults(filteredProducts) {
    // Esconde todo o conteúdo da página inicial
    $(
      "#destaque_full, #conteudo > .cont > .titulo, .listar_produtos:not(#resultado-produtos)"
    ).hide();
    $("#pagina-contato").hide();

    // Reseta a página para a primeira e exibe os produtos
    currentPage = 1;
  ...
  function displayProducts(productsToShow) {
    const productContainer = $("#resultado-produtos");
    ...
    if (productsToShow.length === 0) {
      productContainer.html(
        '<p style="text-align:center; font-size:18px; padding: 40px 0;">Nenhum produto encontrado.</p>'
      );
    ...
    const paginatedItems = productsToShow.slice(
      startIndex,
    ...
    paginatedItems.forEach((product) => {
      const productHtml = `
                <div class="produto" style="border-radius: 10px;">
                    <div class="cont">
                        <a href="#"><img src="${product.image}" alt="${product.name}" class="imgprod"></a><br>
                        <h4 class="nome">${product.code} - ${product.name}</h4>
                        <a href="#" class="comprar"><img src="template/default/imgs/site/botao_comprar.png"></a>
                    </div>
                </div>
            `;
    ...
    productContainer.show();
    setupPagination(productsToShow);
  }

  function setupPagination(fullList) {
    const paginationContainer = $("#paginacao");
    const totalPages = Math.ceil(fullList.length / productsPerPage);

    if (totalPages <= 1) return;

    for (let i = 1; i <= totalPages; i++) {
      const pageButton = $(
        `<button style="padding: 5px 10px; margin: 0 5px; cursor: pointer;">${i}</button>`
      );
      if (i === currentPage) {
        pageButton.css({ "font-weight": "bold", "background-color": "#ddd" });
      }
      pageButton.on("click", function () {
        currentPage = i;
        displayProducts(fullList);
      });
  ...
  $(".btn-mostrar-inicio").on("click", function (e) {
    e.preventDefault();
    showHomepage();
  });

  $(".btn-mostrar-contato").on("click", function (e) {
    e.preventDefault();
    showContactPage();
  });

  $("#menu_site a[data-category]").on("click", function (e) {
    e.preventDefault();
  ...
  $('form[name="busca1"]').on("submit", function (e) {
    e.preventDefault();
    const searchTerm = $("#qb").val().toLowerCase();
...
var Produtos = Class({
  ...
  Comprar: function (cod) {
    ...
    if ($("#opcao_lista").length) {
      if (!$F("opcao_personalizado")) {
        $("html,body").animate(
          { scrollTop: $("#opcao_lista").offset().top },
          500,
          function () {
            VerForm.MostraErro(
              "opcao_personalizado",
              TRAD("Campo Obrigatório"),
              0
            );
          }
        );
    ...
    if ($F("frete_person") && $F("total_carrinho") > 0) {
      if (
        !confirm(
          "Esse produto utiliza um frete especial e somente pode ser enviado sozinho.\nAo continuar os produtos no seu carrinho serão removidos, podendo ser comprados depois em uma nova compra.\nDeseja continuar adicionando esse produto ao carrinho?"
        )
      )
        return false;
    }
  ...
  MaisInfoSub: function (cod, fecha) {
    if (fecha) {
      ...
      $("#maissub_mostra")
        .html($("#maissub_" + cod).html())
        .css({ left: central[0] + "px", top: central[1] + "px" })
        .show();
    }
  },
  ...
  Recomendar: function (cod) {
    ...
    window.open(
      SUBPASTA + "/produtos/recomendar/" + cod,
      "recomendar",
      "width=" +
        larg +
        ",height=" +
  ...
  OpcaoPerson: function (cod, val, editar) {
    ...
    if (editar == "n") {
      ...
      $("#opcao_personalizado")
        .removeClass("msg_person_on")
        .addClass("msg_person_on")
        .animate({ backgroundColor: "#F156B4" }, 500, function () {
          $("#opcao_personalizado").animate(
            { backgroundColor: "#FFFFFF" },
            500,
            function () {
              $("#opcao_personalizado").removeClass("msg_person_on");
            }
          );
        });
      //
...
var Carrinho = Class({
  ...
  Frete: function () {
    ...
    var peso = $F("peso");
    var msg_fora_area = TRAD(
      "Desculpe, mas infelizmente não entregamos na sua região."
    );
    var msg_cep_invalido = TRAD(
      "Desculpe, mas infelizmente não entregamos na sua região."
    );
    MyLib.Aguarde(1);
    $.ajax({
...
var Comprar = Class({
  ...
  ModoCartao: function (valor) {
    ...
    if (this.forma_cartao_atual)
      $("#a_" + this.forma_cartao_atual).removeClass("on");
  ...
  CaracteresCartao: function (caracteres, texto) {
    ...
    $("#msg_cartao_caracteres").html(
      texto.replace("%caracteres%", "<b>" + (caracteres - tam) + "</b>")
    );
  },
});
...
var Cadastro = Class({
  ...
  VerCPF: function (modo) {
    ...
    var cpf = $F("cpf");
    if (!cpf)
      return VerForm.MostraErro(
        "cpf",
        TRAD("Você precisa digitar um CPF/CNPJ")
      );
    if (this.cpfat == cpf) {
    ...
    $.ajax({
      ...
      success: function (t) {
        ...
        if (texto == "ok") {
          ...
          VerForm.MostraErro("cpf", msg_cpf_incorreto);
      },
    });
  },
  // Inicio
  VerEmail: function (modo) {
    ...
    if (!email)
      return VerForm.MostraErro(
        "email",
        TRAD("Você precisa digitar um e-mail")
      );
    ...
    $.ajax({
      ...
      success: function (t) {
        ...
        if (texto == "nao") {
          ...
          VerForm.MostraErro("email", msg_email_cadastrado);
        else if (texto == "erro")
          VerForm.MostraErro("email", msg_email_incorreto);
      },
    });
  },
  VerSenha: function (modo, anima) {
    ...
    if (senha.length < 5) {
      return VerForm.MostraErro(
        "senha",
        TRAD("A senha precisa ter no mínimo 5 caracteres"),
        anima
      );
    }
    if (modo == 1 && senha != resenha) {
      ...
      return VerForm.MostraErro(
        "senha",
        TRAD("As duas senhas estão diferentes"),
        anima
      );
    }
  ...
  Enviar: function () {
    ...
    if ($("#nome").length) {
      ...
      if (!this.Maior()) {
        var texto = TRAD(
          "Desculpe, mas não será possível continuar. Você é menor de %idade%"
        );
    ...
    if (!erro && $("#end_ficha_cod").length) {
      ...
      for (var i = 0; i < campos.length; i++) {
        ...
        if (!$F(cp)) {
          var nome =
            i > 2 ? TRAD("Endereço Pagamento") : TRAD("Endereço Entrega");
  ...
  Iguais: function () {
    ...
    if (!this.D(i, "rua") || !this.D(i, "numero")) {
      ...
      if (!this.D("1", "numero"))
        VerForm.MostraErro("end[1][numero]", erro_msg);
  ...
  ProcuraEnderecoPorCep: function (tipo, form) {
    ...
    if (!cep) {
      return VerForm.MostraErro(
        "end[" + tipo + "][cep]",
        TRAD("Você precisa digitar um CEP")
      );
    } else if (cep.length != 8) {
      return VerForm.MostraErro(
        "end[" + tipo + "][cep]",
        TRAD("O CEP informado está incorreto")
      );
    }
  ...
  ProcurarCepPorEnd: function () {
    ...
    $.ajax({
      url: SUBPASTA + "/index.php",
      data:
        "ajax=1&t=cadastro/procura_end/end&rua=" +
        rua +
        "&uf=" +
        uf +
        "&cidade=" +
  ...
  Completa: function (campo) {
    if (campo) {
      loadScript(
        BASETPL + "/script/lib/jquery-ui-1.8.17.custom.min.js",
        function () {
  ...
  _Auto_Completa: function (id, request, response) {
    ...
    return {
      ...
      type: "POST",
      data:
        "t=cadastro/auto_complete&type=json&modo=" +
        match[2] +
        "&bairro=" +
        bairro +
        "&cidade=" +
...
var VerForm = Class({
  ...
  VerCampos: function () {
        ...
        for (i = 0; i < qb.length; i++) {
          ...
          for (j = 0; j < qbe.length; j++) {
            ...
            if (qbd[0] == "vazio" || qbd[0] == "numeros") {
              VerForm.VerificaCampo(
                $(this).find("input,textarea,select").attr("id"),
                acao,
                qbd[0]
              );
            }
    ...
    if (tem) {
      ...
      if (!$("form.form,comprar.form").attr("onsubmit")) {
        // nome do formulario
        $("form.form,comprar.form").bind("submit", function () {
          return VerForm.Enviar();
        });
      }
    }
  },
  ...
  VerificaCampo: function (id, acao, tipo) {
    if (tipo == "vazio") {
      $(FS(id)).bind(acao, function () {
        VerForm._Ver_Vazio(id);
      });
    } else {
      $(FS(id)).bind(acao, function () {
        VerForm._Ver_Numeros(id);
      });
    }
  ...
  MostraErro: function (id, mensagem, anima) {
    ...
    if (anima) {
      ...
      $("html,body").animate(
        ...
        this.tempo_animacao,
        function () {
          VerForm._MostraErro(id, mensagem);
        }
      );
    } else {
      this._MostraErro(id, mensagem);
  ...
  _MostraErro: function (id, mensagem) {
    ...
    el["div"]
      .find(".cicon")
      .removeClass("ciconloading ciconok")
      .addClass("ciconerro");
    if (mensagem) {
      el["div"].find(".cmsg").html(mensagem);
      el["div"].find(".cicon").bind("mouseenter", function () {
        VerForm._MostraMensagem(id);
      });
      this._MostraMensagem(id);
    }
...
var Entregador = Class({
  ...
  IniciaNacional: function (estado, cidade, bairro, tipo) {
    ...
    $("#end_" + tipo + "_bairro").html(
      '<div class="cinput"><input type="text" name="end[' +
        tipo +
        '][bairro]" id="end[' +
        tipo +
        '][bairro]" value"" /></div>'
  ...
  MontaEstado: function (valor, tipo, padrao) {
    ...
    var js =
      this.modo == "nacional"
        ? "Entregador.MontaCapital(this.value,'" + tipo + "')"
        : "Entregador.MontaCidade(this.value,'" + tipo + "')";
  ...
  MontaCapital: function (valor, tipo, padrao) {
    ...
    if (!valor) {
      ...
      linha = this.Select(
        ...
        cidades,
        padrao,
        "Entregador.MontaCidadeCapitais(this.value,'" + tipo + "')",
        1
      );
    }
    ...
    if (!valor) {
      ...
      if (valor == "capital") {
        ...
        linha = this.Select(
          ...
          vals,
          padrao,
          "Entregador.MontaBairro(this.value,'" + tipo + "')",
          0
        );
      } else {
        linha =
          ...
          '][cidade]" id="end[' +
          tipo +
          '][cidade]" value"" onchange="Entregador.SalvaPadrao(\'' +
          tipo +
          "')\"  /></div>";
        completa = true;
      }
    ...
    if (!valor) {
      ...
      if (!bairros) {
        linha =
          ...
          '][bairro]" value="' +
          atual +
          '" onchange="Entregador.SalvaPadrao(\'' +
          tipo +
          "')\"  /></div>";
        completa = true;
      } else {
      ...
      $(el)
        ...
        .each(function () {
          if ($(this).is(":visible")) {
            ...
            $(this)
              .css("zIndex", 2)
              .fadeOut(f.animacao * 1000, function () {
                $(this).hide();
                setTimeout(function () {
                  $(el).Banner("Muda");
                }, f.intervalo * 1000);
...
$(document).ready(function () {
  ...
  if ($("#carousel").length) {
    loadScript(
      BASETPL + "/script/lib/jquery.featureCarousel.min.js",
      function () {
        $("#carousel").gira();
      }
    );
  ...
  if ($F("MOSTRA_SUB")) {
    $("html,body").animate(
      { scrollTop: $("#id_sub_produtos").offset().top },
      1000,
      function () {
        Produtos.MaisInfoSub($F("MOSTRA_SUB"));
      }
    );
  }
  // animate de cor
